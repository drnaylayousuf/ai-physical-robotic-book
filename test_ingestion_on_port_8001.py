#!/usr/bin/env python3
"""
Test ingestion on port 8001 to avoid port conflicts.
"""
...
def start_server_on_port_8001():
    """Start the backend server on port 8001 to avoid conflicts."""
    # Start the server in a subprocess on port 8001
    server_process = subprocess.Popen([
        sys.executable, '-c',
        'import uvicorn; from backend.main import app; uvicorn.run(app, host="0.0.0.0", port=8001, log_level="info")'
    ])
...
def wait_for_server():
    ...
    print("Waiting for server to be ready on port 8001...")
    for i in range(20):  # Wait up to 20 seconds
        try:
            response = requests.get("http://localhost:8001/api/health", timeout=5)
            if response.status_code == 200:
                print("Server on port 8001 is ready!")
                return True
        except:
            pass
...
def test_diagnostic():
    ...
    try:
        response = requests.get('http://localhost:8001/api/diagnostic/qdrant')
...
def test_qa():
    ...
    try:
        response = requests.post('http://localhost:8001/api/ask',
...
def main():
    ...
    try:
        ...
        server_process = start_server_on_port_8001()

        # Wait for the server to be ready
        if not wait_for_server():
            print("Server on port 8001 failed to start properly")
            return False
